...
describe("Http", () => {
    describe("create", () => {
        ...
        it("stores defaults", () => {
            ...
            const client = httpClient.create(defaults);
    ...
    describe("baseURL", () => {
        ...
        it("does not prepend baseURL to absolute urls", async () => {
            const client = httpClient.create({
                baseURL: "http://should-not-use.com",
            });
    ...
    describe("error handling", () => {
        it("throws on non-2xx status", async () => {
            const client = httpClient.create({ baseURL });
            const err = (await client({ url: "/status/404" }).catch(
                (e) => e,
            )) as HttpError;
        ...
        it("error message includes status code", async () => {
            const client = httpClient.create({ baseURL });
            const err = (await client({ url: "/status/400" }).catch(
                (e) => e,
            )) as HttpError;
    ...
    describe("validateStatus", () => {
        it("accepts all statuses when null", async () => {
            const client = httpClient.create({ baseURL, validateStatus: null });
        ...
        it("uses custom validateStatus", async () => {
            ...
            const client = httpClient.create({ baseURL, validateStatus });

            const ok = await client({ url: "/status/404" });
            expect(ok.status).toBe(404);

            await expect(client({ url: "/status/500" })).rejects.toBeInstanceOf(
                HttpError,
            );
        });
    });
    ...
    describe("timeout", () => {
        it("aborts request after timeout", async () => {
            const client = httpClient.create({ baseURL, timeout: 50 });
